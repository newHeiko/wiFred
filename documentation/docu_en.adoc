...
Set F<n>:: Simulates a function key press and release when switching from forward to center position and again when switching from center position to reverse (and vice versa). This is intended for locos with either a brake function or ESU's Drive Hold on a function key. The speed commands sent to the loco will still be controlled by the speed potentiometer while the direction switch is in the center position. Reversing quicker than within half a second will trigger an emergency stop, otherwise the throttle will continue to send speed commands according to the speed potentiometer position.
